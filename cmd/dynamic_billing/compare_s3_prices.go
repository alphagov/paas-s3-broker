...
import (
	...
	"github.com/aws/aws-sdk-go/aws/session" // https://docs.aws.amazon.com/sdk-for-go/api/aws/session/
	"github.com/aws/aws-sdk-go/service/pricing" // https://docs.aws.amazon.com/sdk-for-go/api/service/pricing/
...
func main() {
	...
	// Get costs from AWS Pricing api
	
	// Note that the AWS Pricing api requires that we pass the region which hosts the pricing api endpoint, i.e. "us-east-1".
	pricingClient := pricing.New(session.New(&aws.Config{
		Region: aws.String("us-east-1"),
	}))
	
	s3Prices, err := getPricing("AmazonS3", pricingClient)
	if err != nil {
		log.Fatalln(err)
	}

	// print s3Prices.PriceList
	fmt.Sprintf("%s", s3Prices)
	...
	// See https://github.com/aws/aws-sdk-go/tree/master/service
...
// }
  ...
  func getPricing(resourceType string, pricingClient *pricing.Pricing) (*pricing.GetProductsOutput, error) {
	// Get pricing data for S3.
	
	productRequest := &pricing.GetProductsInput{
		FormatVersion: aws.String("aws_v1"),
		// TODO: Later on, need to loop through by service code or create separate instances
		ServiceCode: aws.String("AmazonS3"),
	}
	
	// var nextPageToken *string

	// Returns *GetProductsOutput, error
	resp, err := pricingClient.GetProducts(productRequest) // See $GOPATH/src/github.com/aws/aws-sdk-go/service/pricing/api.go:399
	if err == nil {
		fmt.Println(err)
	}
	fmt.Sprintf("%s", resp)
	// print s3Prices.PriceList[0]["serviceCode"]
